...
function Bitplayr() {
  ...
  useEffect(() => {
    const initializePlayer = async () => {
      ...
      try {
        bitPlayrRef.current = (await BitPlayr.createPlayer(
          videoElementId,
          playerOptions,
        )) as IPlayer;
  ...
  const handleBifImageDisplay = (newTime) => {
    ...
    const numOfThumbnailsEachSide = 4; // How many thumbnails you want on each side
      ...
      <div id="control-bar" style={{ display: isAdPlaying ? 'none' : 'flex' }}>
        ...
        <div className="control-bar-buttons">
          <div className="left-controls">
            ...
            <IconButton onClick={handleSkipBackward} color="primary">
              <Replay10Icon />
            </IconButton>
            <IconButton onClick={handleSkipForward} color="primary">
              <Forward10Icon />
            </IconButton>
            <IconButton
              onMouseDown={handleRewindStart}
              onMouseUp={handleRewindEnd}
              onMouseLeave={handleRewindEnd} // In case the cursor leaves the button while holding
            >
              <FastRewindIcon />
            </IconButton>
            <IconButton
              ...
              onMouseLeave={handleFastForwardEnd} // In case the cursor leaves the button while holding
            >
              <FastForwardIcon />
            </IconButton>
